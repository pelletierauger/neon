...
function draw() {
    ...
    amountOfLines = 0;
    let s = 0.01;
    for (let i = 0; i < Math.PI * 150; i += 2) {
        let blur = map(i, 0, Math.PI * 50, 0.01, 0.01);
        let weight = map(i, 0, Math.PI * 50, 0.001, 0.0025);
        lineOptions.weight = weight;
        lineOptions.blurFactor = blur;
        ...
        let x1 = tan(i * 0.001) * cos(frameCount * 0.125 + i + 1) * (i + 1) * s * 0.25;
        let y1 = tan(i * 0.001) * sin(frameCount * 0.125 + i + 1) * (i + 1) * s * 0.25;
