...
// Circular glow instead of triangular glow
holyHills.vertText = `
    // beginGLSL
    attribute vec2 position;
    void main() {
...
void main() {
    ...
    // tri = smoothstep(0.95, 1., tri) + smoothstep(0.5, 1., tri) * 0.5;
    float interior = (smoothstep(0.992, 1., tri));
    float glow = smoothstep(0.94, 1., 1.0-length((uv-vec2(0., 0.025))*0.25));
    float triGlow = smoothstep(0.94, 1., tri) * 1.0;
    // glow = mix(glow, triGlow, 0.25);
    // glow = triGlow;
    // glow *= 1.25;
    // glow = 1.0-length(uv*4.);
    
    // glow = pow(min(1., 1.0/length((uv-vec2(0.0, 0.02))*4.)*0.5), 3.);
    
    tri = glow - (smoothstep(0.998, 1., tri)) + (1.0-smoothstep(1., 1.05, tri)*2.1)*smoothstep(0.998, 1., tri)*0.5;
    // uv -= 0.5;
    // uv.x *= ratio;
    float voro = voronoise(uv*24., 0.75, 0.3);
    float voro2 = voronoise(uv*12., 0.75, 0.3);
